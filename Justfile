...
verify-container container="" registry="ghcr.io/ublue-os" key="":
    ...
    if [[ "${COSIGN_MAJOR}" -lt 3 ]]; then
        ...
        # Checksums from: https://github.com/sigstore/cosign/releases/download/v3.1.1/cosign_checksums.txt
        ...
        if [[ "${UID:-$(id -u)}" -eq 0 ]] || [[ -n "${SUDOIF}" ]]; then
            if ${SUDOIF} install -m 0755 "${COSIGN_INSTALL_PATH}" /usr/local/bin/cosign 2>/dev/null; then

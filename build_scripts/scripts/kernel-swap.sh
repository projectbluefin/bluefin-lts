...
# dm-crypt never autoloads and LUKS systems hang at the initrd passphrase prompt
# (#628); on-demand filesystem, netfilter and crypto modules are affected the same way.
# udev-driven loads are unaffected (libkmod), which is why the disk still shows up.
# Create the path the kernel expects before dracut runs so it lands in the initramfs too.
KERNEL_MODPROBE_PATH="$(sed -n 's/^CONFIG_MODPROBE_PATH="\(.*\)"$/\1/p' "/usr/lib/modules/${CACHED_VERSION}/config")"
